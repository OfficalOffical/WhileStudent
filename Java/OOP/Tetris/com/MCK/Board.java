...
public class Board extends JPanel implements KeyListener{
    ...
    private BufferedImage blocks; // resimi isme atıyor (1)
    ...
    public Board(){
        ...
        try {
            blocks = ImageIO.read(Board.class.getResource ("/tiles.png")); // resmi çağırıyor (1)
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public void paintComponent(Graphics g){
        ...
        for(int row = 0; row < board.length; row++) // Sıra (yatay)(x)
            for(int col = 0; col < board[row].length; col++) // sütun (dikey)(y)
                if(board[row][col] != 0)
        ...
        for(int i = 0; i < boardHeight; i++){
            g.drawLine(0, i*blockSize, boardWidth*blockSize, i*blockSize); // arka plandaki çizgiler
        }
