...
public class Shape {
    ...
    private int tempNum = (int)(Math.random ()*5); ;
    ...
    private int normalSpeed = 600, speedDown = 60, currentSpeed; // u can change speed from here
    ...
    public void update(){
        ...
        if(!(x + deltaX + coords[0].length > 10) && !(x + deltaX < 0)) // limits
        {
        ...
        if(!(y + 1 + coords.length > 20))
        {
            ...
            for(int row = 0; row < coords.length; row++)
                for(int col = 0; col < coords[row].length; col++)
                    ...
                    {
                        if(tempNum==0) {
                            if (board.getBoard ()[y + row + 1][col + x] != 0 || ((y + row + 1) == (19 - y) && (col + x) == 4))
                                collision = true;
                        }
                        else   {
    ...
    public void render(Graphics g){
        ...
        if(tempNum==0) {
            for(int i=0; i < coords.length; i++)
                        g.drawImage (block, 4*board.getBlockSize (), (board.getBlockSize ()*19)-y*board.getBlockSize (), null);
